...
FC 与Conv 底层的实现都是通过GEMM来实现的，例如把几个kernel接成向量，并且组成矩阵然后再扔给GEMM来运算。
所以优化的其中一个方法，那就是调整矩阵的大小来实现GEMM的效率最大化。
http://yongyuan.name/blog/neural-network-layers-understanding.html
